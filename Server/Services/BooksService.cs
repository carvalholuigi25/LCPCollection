...
{
    public class BooksService : ControllerBase, IBooks
    {
        private readonly DBContext _context;
        private IConfiguration _configuration;

        public BooksService(DBContext context, IConfiguration configuration)
        {
            _context = context;
        ...
        {
            var Books = await _context.Books.Where(x => x.Id == id).ToListAsync();
        ...
        {
            return Ok(typeof(Books).GetProperties(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance).Select(x => x.Name).ToList());
        }
        ...
        public async Task<ActionResult<Books>> PostBooks(Books BooksData)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            _context.Books.Add(BooksData);
            await _context.SaveChangesAsync();

            return CreatedAtAction("GetBooksById", new { id = BooksData.Id }, BooksData);
        }
        ...
        public async Task<IActionResult> SearchData([FromQuery] QueryParams qryp)
        {
            try
            {
                ...
                return Ok(new QueryParamsRes<Books>()
                {
                    Data = entities,
                    QueryParams = qparams,
        ...
        public async Task<IActionResult> ResetIdSeed(int rsid = 1)
        {
            try
            {
                ...
                string queryString = $@"DBCC CHECKIDENT('dbo.Books', RESEED, @rsid)";
                ...
                return Ok(new { msg = $"Id of table Books has been reset to {rsid}!", qrycmd = queryString.Replace("@rsid", "" + rsid), res = result, status = 200 });
            }
            catch (Exception ex)
            {
                // Handle exceptions appropriately
        ...
        public async Task<IActionResult> GetLastId()
        {
            try
            {
                string msg = "";
                string connectionString = _configuration["DBMode"]!.Contains("SQLite", StringComparison.OrdinalIgnoreCase) ? _configuration["ConnectionStrings:SQLite"]! : _configuration["ConnectionStrings:SQLServer"]!;
                string queryString = $@"SELECT MAX(id) FROM books";
        ...
        private IQueryable<Books> DoFilterData(IQueryable<Books> queryable, QueryParams qryp)
        {
            Expression<Func<Books, bool>> newexp;

            if (qryp.SortBy!.Contains(nameof(Books.Title)))
            {
            ...
            else if (qryp.SortBy!.Contains(nameof(Books.Description)))
            {
            ...
            else if (qryp.SortBy!.Contains(nameof(Books.CoverUrl)))
            {
            ...
            else if (qryp.SortBy!.Contains(nameof(Books.ImageUrl)))
            {
            ...
            else if (qryp.SortBy!.Contains(nameof(Books.Companies)))
            {
            ...
            else if (qryp.SortBy!.Contains(nameof(Books.Publishers)))
            {
            ...
            else if (qryp.SortBy!.Contains(nameof(Books.Platforms)))
            {
            ...
            else if (qryp.SortBy!.Contains(nameof(Books.TrailerUrl)))
            {
            ...
            else if (qryp.SortBy!.Contains(nameof(Books.Genre)))
            {
            ...
            else if (qryp.SortBy!.Contains(nameof(Books.ReleaseDate)))
            {
            ...
            else if (qryp.SortBy!.Contains(nameof(Books.Rating)))
            {

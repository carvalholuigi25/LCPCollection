...
{
    public class MoviesService : ControllerBase, IMovies
    {
        private readonly DBContext _context;
        private IConfiguration _configuration;

        public MoviesService(DBContext context, IConfiguration configuration)
        {
            _context = context;
        ...
        {
            return Ok(typeof(Movies).GetProperties(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance).Select(x => x.Name).ToList());
        }
        ...
        public async Task<ActionResult<Movies>> PostMovies(Movies MoviesData)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            _context.Movies.Add(MoviesData);
            await _context.SaveChangesAsync();

            return CreatedAtAction("GetMoviesById", new { id = MoviesData.Id }, MoviesData);
        }
        ...
        public async Task<IActionResult> SearchData([FromQuery] QueryParams qryp)
        {
            try
            {
                ...
                return Ok(new QueryParamsRes<Movies>()
                {
                    Data = entities,
                    QueryParams = qparams,
                    Count = entities!.Count,
                    TotalCount = _context.Movies.Count()
                });
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> ResetIdSeed(int rsid = 1)
        {
            try
            {
                ...
                string queryString = $@"DBCC CHECKIDENT('dbo.Movies', RESEED, @rsid)";
                ...
                return Ok(new { msg = $"Id of table Movies has been reset to {rsid}!", qrycmd = queryString.Replace("@rsid", "" + rsid), res = result, status = 200 });
            }
            catch (Exception ex)
            {
                // Handle exceptions appropriately
        ...
        public async Task<IActionResult> GetLastId()
        {
            try
            {
                string msg = "";
                string connectionString = _configuration["DBMode"]!.Contains("SQLite", StringComparison.OrdinalIgnoreCase) ? _configuration["ConnectionStrings:SQLite"]! : _configuration["ConnectionStrings:SQLServer"]!;
                string queryString = $@"SELECT MAX(id) FROM movies";
        ...
        private IQueryable<Movies> DoFilterData(IQueryable<Movies> queryable, QueryParams qryp)
        {
            Expression<Func<Movies, bool>> newexp;

            if (qryp.SortBy!.Contains(nameof(Movies.Title)))
            {
            ...
            else if (qryp.SortBy!.Contains(nameof(Movies.Description)))
            {
            ...
            else if (qryp.SortBy!.Contains(nameof(Movies.CoverUrl)))
            {
            ...
            else if (qryp.SortBy!.Contains(nameof(Movies.ImageUrl)))
            {
            ...
            else if (qryp.SortBy!.Contains(nameof(Movies.Companies)))
            {
            ...
            else if (qryp.SortBy!.Contains(nameof(Movies.Publishers)))
            {
            ...
            else if (qryp.SortBy!.Contains(nameof(Movies.Platforms)))
            {
            ...
            else if (qryp.SortBy!.Contains(nameof(Movies.TrailerUrl)))
            {
            ...
            else if (qryp.SortBy!.Contains(nameof(Movies.Genre)))
            {
            ...
            else if (qryp.SortBy!.Contains(nameof(Movies.ReleaseDate)))
            {
            ...
            else if (qryp.SortBy!.Contains(nameof(Movies.Rating)))
            {
